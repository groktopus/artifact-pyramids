...
#   Layer 1: files matching 01-* or 1-* or *-sources-* or *-atoms-*
#   Layer 2: files matching 02-* or 2-* or *-molecules-* or *-syntheses-*
#   Layer 3: files matching 03-* or 3-* or *-published-* or *-artifacts-*
#
...
L1_PATTERNS=("01-*" "1-*" "*sources*" "*atoms*" "*raw*")
L2_PATTERNS=("02-*" "2-*" "*molecule*" "*synthesis*" "*alloy*")
L3_PATTERNS=("03-*" "3-*" "*publish*" "*artifact*" "*article*" "*draft*" "*post*")
...
check_md_quality() {
    ...
    # Check for markdown files with missing frontmatter
    while IFS= read -r -d '' f; do
        ...
        if ! $has_yaml; then
            # Check if file has substantial content (not a stub)
            local lines
...
check_cross_references() {
    ...
    while IFS= read -r -d '' f; do
        # Find [[wikilinks]] or [atom-NNN] style references
        while IFS= read -r match; do
            ...
            if [[ -n "$ref" ]]; then
                # Check if reference exists somewhere in the project
                local found_ref
                found_ref=$(grep -rl "$ref" "$dir" --include='*.md' 2>/dev/null | head -1 || true)
                if [[ -z "$found_ref" ]]; then
                    findings+="  ⚠  ${f##*/} references '$ref' which is not found in any file\n"
...
# Run checks
L1_ISSUES=$(check_md_quality "$PROJECT_DIR" 2>/dev/null | tail -1 || echo 0)
L1_FINDINGS=$(check_md_quality "$PROJECT_DIR" 2>/dev/null | head -n -1 || true)
MISSING_REFS=$(check_cross_references "$PROJECT_DIR" 2>/dev/null | tail -1 || echo 0)
REF_FINDINGS=$(check_cross_references "$PROJECT_DIR" 2>/dev/null | head -n -1 || true)
    ...
    cat <<EOF
{
  ...
  "layers": {
    "1": { "count": $L1_COUNT, "present": $L1_PASS, "quality_issues": $L1_ISSUES },
    "2": { "count": $L2_COUNT, "present": $L2_PASS, "quality_issues": 0 },
    "3": { "count": $L3_COUNT, "present": $L3_PASS, "quality_issues": 0 }
  },
  "missing_references": $MISSING_REFS,
  "pyramid_health": $($L1_PASS && $L2_PASS && $L3_PASS && echo '"complete"' || echo '"incomplete"')
}
EOF
    exit 0
fi
...
echo "  Layer 1 — Raw Sources & Atoms"
if $L1_PASS; then
...
echo "  Layer 2 — Molecules & Syntheses"
if $L2_PASS; then
...
echo "  Layer 3 — Published Artifacts"
if $L3_PASS; then
...
elif $L1_PASS || $L2_PASS || $L3_PASS; then
    echo -e "    ${YELLOW}◐ Partial — $($L1_PASS || echo "L1 missing; ") $($L2_PASS || echo "L2 missing; ") $($L3_PASS || echo "L3 missing")${NC}"
    echo ""
    echo "  Suggested Next Steps:"
    if ! $L1_PASS; then echo "    - Add raw sources and atoms (01-sources/, 01-atoms/)" ; fi
    if ! $L2_PASS; then echo "    - Synthesize molecules from existing atoms (02-molecules/)" ; fi
    if ! $L3_PASS; then echo "    - Polish a published artifact from molecules (03-published/)" ; fi
else
    echo -e "    ${RED}✗ Empty — no pyramid artifacts found${NC}"
    echo ""
    echo "  This directory doesn't appear to use the Artifact Pyramid structure."
    echo "  See assets/pyramid-template.md for a starting template."
fi
echo ""
